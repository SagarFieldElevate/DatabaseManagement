...
def delete_file_from_github(filename, repo_name, branch, upload_path, token, sha=None, max_retries=3):
    ...
    for attempt in range(max_retries):
        # Fetch the latest SHA each attempt to avoid using stale data
        ...
        if get_resp.status_code != 200:
            raise Exception(
                f"❌ Unable to fetch file SHA: {get_resp.status_code} - {get_resp.text}"
            )
        ...
        if delete_resp.status_code == 409 and attempt < max_retries - 1:
            # Branch or file changed; retry with a fresh SHA
            continue

        if delete_resp.status_code == 404:
            # File disappeared between fetch and delete
            return
